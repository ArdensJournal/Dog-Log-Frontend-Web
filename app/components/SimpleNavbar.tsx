        ...
        <div className="flex gap-4">
          {isSignedIn && NAV_SECTIONS.map(section => (
            <div key={section.id} className="relative">
              <Button
                type="button"
                className={`bg-white text-indigo-700 font-bold px-5 py-3 rounded-xl border-none shadow-none text-base transition-colors duration-150 hover:bg-indigo-100 hover:text-indigo-700 dark:bg-transparent dark:text-indigo-400 dark:hover:bg-indigo-900/50 ${expandedSection === section.id ? 'underline' : ''}`}
        ...
        <div className="fixed inset-0 z-50 bg-white dark:bg-gray-900 flex flex-col">
          <div className="flex items-center px-4 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-md">
            ...
            <h2 className="flex-1 text-center text-lg font-bold text-indigo-700 dark:text-indigo-300 flex items-center gap-2 justify-center">
              ...
              {mobileTab === 'Community' && (<><MdGroups className="inline text-indigo-700 dark:text-indigo-300" /> Community</>)}
              {mobileTab === 'Notifications' && (<><MdNotifications className="inline text-indigo-700 dark:text-indigo-300" /> Notifications</>)}
              {mobileTab === 'Services' && (<><MdBusiness className="inline text-indigo-700 dark:text-indigo-300" /> Services</>)}
            </h2>
            <span className="w-8" />
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 pb-20">
            {isSignedIn ? (
              <>
                ...
                {mobileTab === "Community" && (
                  <ul className="space-y-1">
                    <li><Link href="/forum" className="block px-6 py-2 rounded-lg font-semibold transition text-sm" onClick={() => setIsMobileDrawerOpen(false)}>Community</Link></li>
                  </ul>
                )}
                ...
                {mobileTab === "Services" && (
                  <ul className="space-y-1">
                    <li><Link href="/services" className="block px-6 py-2 rounded-lg font-semibold transition text-sm" onClick={() => setIsMobileDrawerOpen(false)}>Services</Link></li>
                  </ul>
                )}
                {mobileTab === "Profile" && (
          ...
          {isSignedIn && (
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 flex justify-around items-center h-16 shadow-lg">
              ...
              <button className={`flex flex-col items-center justify-center px-2 py-1 text-xs font-semibold transition ${mobileTab === 'Community' ? 'text-indigo-600' : 'text-gray-500 dark:text-gray-300 hover:text-indigo-700 dark:hover:text-indigo-400'}`} onClick={() => setMobileTab('Community')}>
                <MdGroups className="text-lg mb-1" />
                <span>Community</span>
              </button>
              <button className={`flex flex-col items-center justify-center px-2 py-1 text-xs font-semibold transition ${mobileTab === 'Notifications' ? 'text-indigo-600' : 'text-gray-500 dark:text-gray-300 hover:text-indigo-700 dark:hover:text-indigo-400'}`} onClick={() => setMobileTab('Notifications')}>
                <MdNotifications className="text-lg mb-1" />
                <span>Notifications</span>
              </button>
              <button className={`flex flex-col items-center justify-center px-2 py-1 text-xs font-semibold transition ${mobileTab === 'Services' ? 'text-indigo-600' : 'text-gray-500 dark:text-gray-300 hover:text-indigo-700 dark:hover:text-indigo-400'}`} onClick={() => setMobileTab('Services')}>
                <MdBusiness className="text-lg mb-1" />
                <span>Services</span>
              </button>
