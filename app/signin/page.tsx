  ...
  async function handleGoogleAuth(googleToken: string) {
    try {
      ...
      try {
        ...
        console.log('🔑 Your client ID:', process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID);
        console.log('❓ Match?', payload.aud === process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID);
      } catch (decodeError) {
        console.error('Failed to decode token:', decodeError);
      }
      
      // Backend expects Authorization header WITHOUT "Bearer " prefix
      const response = await fetch(process.env.NEXT_PUBLIC_BACKEND_URL!, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': googleToken, // Send token directly without "Bearer " prefix
        },
        body: JSON.stringify({
          query: `
      ...
      if (data.errors) {
        console.log('❌ Backend Error:', data.errors[0].message);
        setError(`Google authentication failed: ${data.errors[0].message}`);
        return;
      }

      // Success! Store tokens and redirect
      localStorage.setItem('accessToken', data.data.signInByGoogle.accessToken);
      localStorage.setItem('refreshToken', data.data.signInByGoogle.refreshToken);
      router.push('/');

    } catch (err) {
      console.error('Google Auth Error:', err);
      setError('Failed to authenticate with Google - network error');
    }
  }
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-100 via-white to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-indigo-900 p-8">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 w-full max-w-md">
        <h1 className="text-3xl font-bold text-indigo-700 dark:text-indigo-400 mb-4 text-center">Sign In to Dog Log</h1>
        ...
        <div className="text-center">
          ...
          <button
            type="button"
            onClick={() => signInWithGoogle(handleGoogleAuth, setError)}
        ...
        <p className="mt-4 text-center text-gray-600 dark:text-gray-400">
          ...
          <Link href="/signup" className="text-indigo-500 dark:text-indigo-400 underline font-semibold">
            Sign Up
          </Link>
        </p>
      </div>
