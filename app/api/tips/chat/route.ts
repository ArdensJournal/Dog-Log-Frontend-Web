...
function generateContextualResponse(question: string, insights: string[]): string {
  const lowerQuestion = question.toLowerCase();

  // Check for specific keywords and provide relevant responses
  if (lowerQuestion.includes('weight') || lowerQuestion.includes('overweight') || lowerQuestion.includes('diet')) {
    const weightInsight = insights.find(i => i.toLowerCase().includes('weight') || i.toLowerCase().includes('diet'));
    if (weightInsight) {
  ...
  if (lowerQuestion.includes('exercise') || lowerQuestion.includes('activity') || lowerQuestion.includes('walk')) {
    const exerciseInsight = insights.find(i => i.toLowerCase().includes('exercise') || i.toLowerCase().includes('activity') || i.toLowerCase().includes('walk'));
    if (exerciseInsight) {
  ...
  if (lowerQuestion.includes('food') || lowerQuestion.includes('feed') || lowerQuestion.includes('nutrition')) {
    const nutritionInsight = insights.find(i => i.toLowerCase().includes('food') || i.toLowerCase().includes('nutrition') || i.toLowerCase().includes('diet'));
    if (nutritionInsight) {
  ...
  if (lowerQuestion.includes('vaccine') || lowerQuestion.includes('vaccination') || lowerQuestion.includes('shot')) {
  ...
  if (lowerQuestion.includes('vet') || lowerQuestion.includes('doctor') || lowerQuestion.includes('checkup')) {
  ...
  // Generic response
  return `Thank you for your question. Based on the health insights generated for your dog, I recommend discussing specific concerns with your veterinarian. The insights provided are general recommendations, and your vet can provide personalized medical advice. ${insights.length > 0 ? `Here's one of the key insights: "${insights[0]}"` : ''}`;
}
