  ...
  try {
    ...
    if (contentType?.includes("multipart/form-data")) {
      ...
      try {
        ...
        console.log(
          "🔍 Parsed operations variables:",
          JSON.stringify(parsedOps.variables, null, 2)
        );
      } catch (e) {
      ...
      if (!response.ok) {
        console.error(
          "❌ Backend createDog request failed:",
          response.status,
          responseText
        );
        return Response.json(
      ...
      try {
        ...
        console.error(
          "❌ Failed to parse createDog response as JSON:",
          parseError
        );
        return Response.json(
      ...
      console.log(
        "🚀 Sending JSON createDog mutation:",
